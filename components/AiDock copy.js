...
import { ChevronDown, Maximize2, Expand, Plus, ArrowUp, Loader2 } from "lucide-react";

const cx = (...c) => c.filter(Boolean).join(" ");

/* ----------------------------- Shadows (different recipes) ----------------------------- */
const SH_DOCK = "shadow-[0_1px_0_rgba(0,0,0,0.06),0_30px_70px_rgba(0,0,0,0.16)]";
...
export default function AiDock({ tabData }) {
  ...
  const heightClass = dockSize === "sm" ? "h-44" : dockSize === "lg" ? "h-[520px]" : "h-64";
    ...
    <div className={cx("bg-white", compact ? "p-3" : "p-3 border-t border-zinc-200/70")}>
      <div className="flex items-center gap-2 rounded-2xl border border-zinc-200/80 bg-white px-3 py-2">
        ...
        <button
          ...
          className={cx(
            ...
            input.trim() ? "bg-zinc-900 hover:bg-zinc-800" : "bg-zinc-200 cursor-not-allowed",
            "transition-colors"
          )}
          aria-label="Send"
          disabled={!input.trim()}
      ...
      {compact ? (
        <div className="mt-2 flex items-center justify-between">
          ...
          <div className="text-[11px] text-zinc-500">AI Assistant • {tabData.title}</div>
        </div>
      ) : (
